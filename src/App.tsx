...
function Character({ campaign, onUpdate, onComplete }: { campaign: Campaign; onUpdate: (campaign: Campaign) => void; onComplete: () => void }) {
  ...
  const chooseMastery = (index:number, name:string) => {
    const option=MASTERY_OPTIONS.find(value=>value.name===name);
    updateMastery(index,{name,feature:option?.featureName?`${option.featureName} — ${option.featureSummary}`:"",tierOneAbility:option?.tierOneAbility??""});
  };
  ...
  return <section className="character-workspace">
    ...
    <div className="panel mastery-panel"><div className="section-kicker"><span>GRAVEBOUND MASTERIES</span><PageRef pages="21–61"/></div><h2>Two paths into the dark</h2><p>Choose two different Masteries. Core selections automatically record their Feature and starting Tier 1 Ability; the short reminders guide play without replacing the book.</p><div className="mastery-choices">{campaign.masteries.map((mastery,index)=>{const option=MASTERY_OPTIONS.find(value=>value.name===mastery.name);const other=campaign.masteries[index===0?1:0].name;return <article key={mastery.id}><label>Mastery {index+1}<select value={mastery.name} onChange={e=>chooseMastery(index,e.target.value)}><option value="">Choose a Mastery…</option>{mastery.name&&!MASTERY_OPTIONS.some(value=>value.name===mastery.name)&&<option value={mastery.name}>{mastery.name}</option>}{MASTERY_OPTIONS.map(value=><option key={value.name} value={value.name} disabled={value.name===other}>{value.name}</option>)}</select></label>{option&&<><div className="mastery-reference"><PageRef pages={option.pages}/><p><strong>Focus</strong>{option.focus}</p></div>{option.tierOneSummary&&<p className="mastery-start"><strong>Tier 1 · {option.tierOneAbility}</strong>{option.tierOneSummary}</p>}</>}<label>Feature<textarea value={mastery.feature} placeholder="Choose a core Mastery to fill this automatically…" onChange={e=>updateMastery(index,{feature:e.target.value})}/></label><label>Tier 1 Ability<input value={mastery.tierOneAbility} placeholder="Choose a core Mastery to fill this automatically…" onChange={e=>updateMastery(index,{tierOneAbility:e.target.value})}/></label></article>})}</div><p className="source-note">The 20 core Masteries use Gravebound Edition pages 22–61. Fatebender and Caustic Blade remain available as expansion entries, with editable fields until The Book of Masteries is added.</p></div>
